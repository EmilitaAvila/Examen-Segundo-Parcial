...
public class EliminarProductos extends JDialog {
	...
	public EliminarProductos(Dialog owner, boolean modal, DefaultTableModel model, 
			ArrayList<Productos> productos, JLabel lblSubtotal, JLabel lblIVA, JLabel lblTotal) {
		super(owner, modal);
		setType(Type.POPUP);
		setBounds(100, 100, 481, 300);
		getContentPane().setLayout(new BorderLayout());
		contentPanel.setBackground(new Color(183, 255, 183));
		...
		if (!FrmProductos.getProductos().isEmpty()) {
		...
		
		{
			...
			buttonPane.setBackground(new Color(255, 200, 145));
			buttonPane.setLayout(new FlowLayout(FlowLayout.RIGHT));
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				JButton okButton = new JButton("OK");
				okButton.setIcon(new ImageIcon("C:\\Users\\navar\\Downloads\\cheque.png"));
				okButton.setFont(new Font("Lucida Sans Typewriter", Font.BOLD, 12));
			...
			}
			{
				...
				cancelButton.setFont(new Font("Lucida Sans Typewriter", Font.BOLD, 12));
	...
	private void agregarProducto(ArrayList<Productos> productos) {
	...
	private void eliminarProducto(DefaultTableModel model, ArrayList<Productos> productos, JLabel lblSubtotal, JLabel lblIVA, JLabel lblTotal) {
		...
		for (Productos producto: FrmProductos.getProductos()) {
